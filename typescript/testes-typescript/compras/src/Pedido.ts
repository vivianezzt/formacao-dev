...
export class Pedido {
    ...
    if (this.finalizado) {  // Corrigido o erro de digitação para 'if'
        throw new Error("O pedido já foi finalizado");
    }
	...
	finalizarPedido(): void {
		if(this.produtos.length === 0){
			throw new Error("Não é possivel finalizar pedido vazio")
		}
    ...
    const produtosDescritos = this.produtos
        .map(
            (p) => `${p.produto.getDescricao()}, Quantidade: ${p.quantidade}`
        )
        .join("\n");
