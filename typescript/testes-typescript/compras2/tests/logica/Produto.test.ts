...
describe("Classe Produto", () => {
  ...
  describe("Ao reduzir o estoque", () => {
    ...
    test("Deve lançar um erro ao tentar retirar um valor negativo", () => {
      ...
      const p = new ProdutoBuilder().padrao().comEstoque(estoque).build();
      const alteracao = () => p.reduzirEstoque(tirar);
      expect(alteracao).toThrow("Não é possível alterar estoque");
    });
  });
  ...
  describe("Ao converter para objeto", () => {
	const desconto = 10
    test("Deve retornar um objeto com todas as propriedades", () => {
      const p = new ProdutoBuilder()
        .comNome(nome)
        .comPrecoOriginal(precoOriginal)
        .comEstoque(estoque)
    ...
    test("Deve retornar um objeto com todas desconto indefinido", () => {
		const p = new ProdutoBuilder()
        .comNome(nome)
        .comPrecoOriginal(precoOriginal)
        .comEstoque(estoque)
